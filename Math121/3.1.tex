...
\begin{solution}
    \bbni
    \begin{enumerate}
        \item \textbf{Step 0:}  $q_1 = 0$, \, $q_2 = 0$, \, $p = x^2y + xy + y - x - 2$, \, $r = 0$
        
        \textbf{Step 1:} 
        $q_1 = x $, \, $q_2 = 0$, \, $p = xy + y - 2$, \, $r = 0$

        \textbf{Step 2:} 
        $q_1 = xy$, \, $q_2 = 0$, \, $p = y - 1 $, \, $r = 0$

        \textbf{Step 3:} 
        $q_1 = xy$, \, $q_2 = y$, \, $p = 0$, \, $r = 0$

        \item  \textbf{Step 0:}  $q_1 = 0$, \, $q_2 = 0$, \, $p = xy^2 - x$, \, $r = 0$
        
        \textbf{Step 1:} 
        $q_1 = y $, \, $q_2 = 0$, \, $p = -x + y$, \, $r = 0$

        \textbf{Step 2:} 
        $q_1 = y $, \, $q_2 = 0$, \, $p = y$, \, $r = -x$

        \textbf{Step 3:} 
        $q_1 = y $, \, $q_2 = 0$, \, $p = 0$, \, $r = -x + y$
        ...
        \item From part 3 we know that $xy^2 - x \in \langle y^2 - 1 \rangle \implies xy^2 - x \in \langle xy - 1, y^2 - 1 \rangle$. 
        
        \item If the remainder is zero the polynomial is in the ideal generated by the divisors. The converse is not true; that is if the remainder is non-zero, we can't conclude that the polynomial is not in the ideal.  
    \end{enumerate}
\end{solution}
...
\begin{solution}
    \bbni
    \begin{enumerate}
        \item We will show this by induction on the number of steps. 
        
        \textbf{Base Case:} It holds initially since $p = f$ and $q_i = r = 0$. 

        \textbf{Inductive Step:} Assume that the the invariant $f = q_1g_1 + \cdots + q_sg_s + p + r$ is maintained for the first $n-1$ steps, then at the $n$-th step, we either,
        \begin{itemize}
            \item Replace $q_i \leftarrow q_i + \frac{\LT(p)}{\LT(g_i)}$ and $p \leftarrow p - \frac{\LT(p)}{\LT(g_i)}g_i$, so the sum $q_ig_i + p$ is unchanged.
            \item Or replace $r \leftarrow r + \LT(p)$ and $p \leftarrow p - \LT(p)$, so $p + r$ is unchanged.
        \end{itemize}

        In both cases the invariant is preserved, thus we always have $f = q_1g_1 + \cdots + q_sg_s + p + r$.

        \item If a $g_i$ divides the leading term, we multiply it with $q_i$ and subtract the leading term, if no $g_i$ divides the leading term, we subtract it from $p$ (and add it to $r$), either ways each pass through the \texttt{while} loop removes the leading monomial and thus the leading monomial of $p$ strictly decreases with respect to the given monomial order.
        
        \item Since every strictly decreasing sequence of monomials terminates (Question 1(d)), $p$ must also terminate, i.e. $p = 0$ eventually and the algorithm terminates. 
        
        \item From part 1 we know that we always have $f=q_1g_1+\cdots+q_sg_s+p +r$, since on termination we have $p =0$ we get, 
        \[f=q_1g_1+\cdots+q_sg_s+r\]
        Moreover, since the algorithm only adds $\LT(p)$ to $r$ when $\LT(g_i) \nmid \LT(p)$ for all $i$ no monomial in $r$ is divisibile by any of $\LM(g_1),\ldots,\LM(g_s)$. 

        \item If $r=0$ we have $f=q_1g_1+\cdots+q_sg_s$ which is precisely the condition for ideal membership. 
    \end{enumerate}
\end{solution}
...
%             \item $\langle x^2,xy^3,z\rangle$ - is a monomial ideal. 
%             \item $\langle x+y,y^2\rangle$ 
%             \item $\langle x^2-y,y^3\rangle$
%             \item $\langle x^2,xy,y^4+x\rangle.$
%         \end{enumerate}
%         \item If
%     \end{enumerate}
% \end{solution}

...
\begin{problem}{1}
    \textbf{Basic Open Sets.} Let $R$ be a ring and $f,g \in R$.
    \begin{enumerate}
...
\begin{problem}{2}
    \textbf{Examples of Basic Opens.} Explicitly describe the basic open set $D(f) \subset \Spec(R)$ in each of the following cases. 
    \begin{multicols}{2}
    \begin{enumerate}
        \item $R=\K[x]$ and $f=x$.
...
\begin{problem}{3}
    \textbf{Induced Map by Localization.}  Let $U\subset R$ be a multiplicatively closed set and let $\eta:R\to U^{-1}R$ be the localization map. In this exercise we will study the induced map $\eta^{\#}:\Spec(U^{-1}R) \to \Spec(R)$. 
    \begin{enumerate}

...
\item \textbf{Reviewing leading terms and initial ideals.} Let $S=\K[x,y,z]$.
\begin{enumerate}[label=(\alph*),ref=\theenumi\alph*]
    ...
    \item Let $I=\langle xy-1,y^2-1\rangle\subset \K[x,y]$ and use lex order with $x>y$. Show that
    \[
...
\begin{enumerate}[label=(\alph*),ref=\theenumi\alph*]
    ...
    \item Let $I=\langle xy-1,y^2-1\rangle\subset\K[x,y]$ with lex order $x>y$. Divide $x-y$ by the ordered list $(xy-1,y^2-1)$. Explain why the answer is consistent with part (b).
\end{enumerate}
...
For monomial ideals, the monomials outside the ideal form a ``staircase.'' For example if we consider the ideal $I=\langle x^5, x^{3}y^{2}, xy^{4}, y^{6}\rangle$ in $\K[x,y]$ we can picture it as via the diagram below. The monomials in $I$ are shown in red while those that are non-zero in the quotient $S/I$ are shown in blue.
...
Let $I\subset S$ be an ideal and fix a monomial order. The residue classes of the standard monomials form a $\K$-basis of $S/I$.
\end{theorem}
...
\begin{enumerate}[label=(\alph*),ref=\theenumi\alph*]
    \item Show that the remainder obtained by dividing any $f\in S$ by $G$ is a $\K$-linear combination of standard monomials.
    
    \item Deduce that the residue classes of the standard monomials span $S/I$.
    
    \item Suppose $h$ is a nonzero $\K$-linear combination of standard monomials. Prove that $h\notin I$. Hint: What can you say about $\LT(h)$ if $h\in I$?
...
\begin{enumerate}[label=(\alph*),ref=\theenumi\alph*]
    \item $M=\langle x^3,x^2y,y^4\rangle\subset\K[x,y]$.
    
    \item $M=\langle x^2,xy,y^3\rangle\subset\K[x,y]$.
    
    \item $M=\langle x^2z,xyz,y^2z,z^3\rangle\subset\K[x,y,z]$.
    
    \item Let $I=\langle x-y^2,y^3-1\rangle\subset\K[x,y]$ with lex order $x>y$. Verify that the displayed generators are a reduced Gr\"obner basis. List $\std_<(I)$ and compute $\dim_{\K}(S/I)$.
...
\begin{enumerate}[label=(\alph*),ref=\theenumi\alph*]
    \item Prove that $S/I$ is finite-dimensional as a $\K$-vector space if and only if $\std_<(I)$ is finite. In this case,
    \[
    \dim_{\K}(S/I)=\#\std_<(I).
    \]
    
    \item Let $M\subset S$ be a monomial ideal. Prove that $S/M$ is finite-dimensional over $\K$ if and only if, for every variable $x_i$, there exists an integer $a_i\geq1$ such that $x_i^{a_i}\in M$.
    
    \item Deduce that $S/I$ is finite-dimensional over $\K$ if and only if, for every variable $x_i$, there exists $a_i\geq1$ such that $x_i^{a_i}\in \init_<(I)$.
...
\item \textbf{$S$-polynomials by hand.} Work in $\K[x,y]$ with lex order $x>y$.
\begin{enumerate}[label=(\alph*),ref=\theenumi\alph*]
...
\item \textbf{Another full Buchberger computation.} Work in $\K[x,y]$ with lex order $x>y$. Let
\[
...
\begin{enumerate}[label=(\alph*),ref=\theenumi\alph*]
    ...
    \item Use this criterion to show that $x\in \sqrt{\langle x^2,xy\rangle}\subset \K[x,y]$.

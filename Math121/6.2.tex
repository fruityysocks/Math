...
\begin{problem}{1} \textbf{Local Vanishing for Modules.} 
    
    
    Let $R$ be a ring and $M$ an $R$-module. If $\cS \subset M$ is any subset, the \emph{annihilator} of $\cS$ is the ideal:
    \[
...
\begin{enumerate}
    ...
    \item Let $n\geq 1$ be an integer and consider multiplication by $n$ as a map of $\Z$-modules
    \[
    \begin{tikzcd}[column sep=large]
    \Z \arrow[r, "\times n"] & \Z.
    \end{tikzcd}
    \]
    For which prime ideals $\langle p\rangle\in \Spec(\Z)$ does the localized map become an isomorphism? For which prime ideals is it injective? Surjective?
\end{enumerate}
\end{enumerate}
...
A\adj(A) = \adj(A)A=\det(A)\Id_{n\times n}.
\end{equation}
In this problem let $S=\Z[x_{i,j}\mid 1\leq i,j\leq n]$ be the polynomial ring in $n^2$ variables. Let $X=(x_{i,j})$ be the generic $n\times n$ matrix with entries in $S$.
\begin{enumerate} 
\item Show that an arbitrary ring has a canonical structure of a $\Z$-algebra.
\item Given an $n \times n$ matrix $A=(a_{i,j})$ with entries in $R$ show that there is a well-defined morphism of $\Z$-algebras:
\[
\begin{tikzcd}[row sep = .3em, column sep = 3.5em]
S \arrow[r, "\ev_{A}"] & R \\
...
\begin{enumerate}
    \item $R=\Z$ and $A=\begin{pmatrix}1&2\\3&4\end{pmatrix}$.
    \item $R$ is any ring and $A=\begin{pmatrix}0&-1\\1&0\end{pmatrix}$.
    \item $R=\Z/6\Z$ and $A=\begin{pmatrix}2&1\\0&2\end{pmatrix}$.
...
\begin{enumerate}
    \item Let $R=\Z$,  $I=\langle 2\rangle$, and  $M=\Z/3\Z$. Verify that $IM=M$ and find $a\in I$ such that $(1-a)M=0$.
...
\begin{enumerate}
    ...
    \item Compute $\Jac(R)$ for the following rings: $R=\K$ a field, $R=\Z$, and $R=\Z/12\Z$.
...
\begin{enumerate}
    ...
    is an isomorphism, then $\phi$ is surjective. Give an example showing that $\phi$ need not be injective.
        \item Give an example showing that an injective map of finitely generated modules can become non-injective after reducing modulo $\fm$. Hint: Use multiplication by $x$ on $\K[x]_{\langle x\rangle}$.
\end{enumerate}
...
\item \textbf{Computing Minimal Numbers of Generators.} Let $R=\K[x,y]_{\langle x,y\rangle}$ with maximal ideal $\fm=\langle x,y\rangle R$ and residue field $\K$.
\begin{enumerate}
    ...
    \item Let $S=\Z_{\langle p\rangle}$ and $M=S/\langle p^4\rangle\oplus S/\langle p^2\rangle$. Compute the minimal number of generators of $M$ as an $S$-module.
\end{enumerate}
...
\begin{enumerate}
    ...
    \]
    \item Explain why the analogous statement for injectivity is false by revisiting the multiplication-by-$x$ example over $\K[x]_{\langle x\rangle}$.

...
Your **midterm** already tested the first half of the course: fundamental group, covering spaces, retractions, Brouwer/Borsuk–Ulam, π₁(S¹), π₁(RPⁿ), homotopy equivalence. A final is usually cumulative, but the *new* material — and where your later notes and homology homework concentrate — is **homology and its machinery**. Suggested split:

- **~60%** homology: simplicial/singular, reduced, LES of a pair, good pairs, excision, equivalence of simplicial & singular, Mayer–Vietoris, degree, Euler characteristic, suspension.
- **~40%** review: π₁, covering spaces, van Kampen, the standard π₁ computations.

The single highest-yield skill is **computing homology from a Δ-complex or via an exact sequence**, because it shows up in your notes (S¹, T², RP²) *and* your homework (RP² again, Dⁿ/Sⁿ⁻¹, Δⁿ with faces identified).
